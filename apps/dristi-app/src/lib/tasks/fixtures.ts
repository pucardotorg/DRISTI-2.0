...
/** ISO for `days` from NOW at a local hour (bands are computed on the local calendar). */
...
export function makeTask(over: Partial<Task> = {}): Task {
  ...
  const finalising = ["sign", "pay", "submit", "fix-defects"].includes(kind);
  return {
    id: "t-1",
    caseId: kase.id,
    kind,
    ...
    isBlocking: false,
    createdAt: at(-3),
    requiresSignatory: finalising,
    systemObservable: finalising,
    status: "open",
    history: [],
    ...over,
  };
