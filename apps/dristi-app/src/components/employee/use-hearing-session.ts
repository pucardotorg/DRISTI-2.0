 ...
 * list, and the cause list is what unmounts when the bench follows a cause title to
 * the case overview or opens the order composer. The server snapshot is the same empty
 * session the module starts at, so the first paint and hydration agree and there is
 * nothing to suppress.
 */
