...
} from "@/components/home/app-shell";
...
export function AdvocateScreen({
  ...
  /** Deep-link target — the shared shell's "Join a Case" lands on the join view. */
  initialView?: "home" | "join" | "cases" | "team";
}) {
  const router = useRouter();
  const [locale, setLocale] = React.useState<Locale>(initialLocale);
  const [view, setView] = React.useState<"home" | "join" | "cases" | "team">(
    initialView,
  );
  ...
  const [dialogMode, setDialogMode] = React.useState<"summons" | "manual">(
    "manual",
  );
  ...
  const hasJoinedSummons = entries.some(
    (entry) => entry.joinCase.cnr === summonsCase?.cnr,
  );
    ...
    setNotifications((current) => [
      {
        ...
        title: fillCopy(
          result.joined
            ? advDialog.notifJoinedTitle
            : advDialog.notifRequestTitle,
          locale,
  ...
  const firstName = ADVOCATE_PROFILE_NAME.replace(/^Adv\.\s*/, "").split(
    " ",
  )[0];
          ...
          <main className="mx-auto flex w-full max-w-3xl flex-1 flex-col gap-8 px-4 py-8 md:px-6 md:py-10">
            ...
            {entries.length ? (
              <section
                ...
                aria-label={pick(advJoinPage.recentHeading, locale)}
              >
                <h2 className="text-title-s font-semibold">
                  {pick(advJoinPage.recentHeading, locale)}
                </h2>
                      ...
                      <CardContent className="flex h-full flex-col gap-4">
                        <div className="flex flex-col items-start gap-3">
                          <Badge variant={entry.joined ? "success" : "warning"}>
                            {pick(
                              entry.joined
                                ? advJoinPage.statusJoined
                                : advJoinPage.statusPending,
                              locale,
                            )}
                          </Badge>
        ...
        {notice ? (
          <div className="mx-auto w-full max-w-5xl px-4 pb-8 md:px-6">
            <Alert variant="info">
              ...
              <AlertDescription>
                {pick(shell.prototypeNav, locale)}
              </AlertDescription>
            </Alert>
          </div>
        ) : null}
