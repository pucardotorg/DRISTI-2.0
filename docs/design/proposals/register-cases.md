...
| D62 | **The state sits above the question, not beside it**, on a line of its own; the outcome's mark is a circle either way (a tick for registered, a left arrow in a ring for sent back), and the header keeps its rule in both states so the question reads as a header over the answer. | Owner: "the tag should be on top or below the heading"; "I like that in the success state it's a round icon" |
