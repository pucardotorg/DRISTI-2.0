...
| 2026-08-21 | **The header became the breadcrumb** (owner feedback). The screen publishes *Tasks › the task › Scrutiny return* to the app top bar — the same `Breadcrumbs` contract every other task action uses — and the in-page header collapses to one slim identity strip: `text-title-s` name over one caption line (parties · number · court · returned date). The "Back to tasks" button is gone; the way back is the crumb. ~110px of chrome became ~70. | ui-designer (owner feedback) |
| 2026-08-21 | **The queue sorts in the form's order, not the officer's numbering** (owner feedback: "index order"). `formOrder` in `lib/tasks/defects.ts` — walking order, then instance, then `n` as tiebreak — is applied once where the screen reads the task's defects, so the queue, the counts and the defect the screen opens on all follow. The memo's own numbers stay on the record; they say who wrote what, not where the work is. Two tests. | ui-designer (owner feedback) |
